    ...
    if request.method == 'GET':
        try:
            ...
            for message_id, message_data in redis_data.items():
                try:
                    ...
                    conn.hdel(redis_key, message_id)
                    continue
            logger.debug(f"[API] Returning messages for user {request.user.id}: {messages}")
            return JsonResponse({'status': 'success', 'messages': messages})
        except Exception as e:
            logger.error(f"[API] Critical error fetching messages for user {request.user.id}: {str(e)}", exc_info=True)
        ...
        try:
            ...
            if conn.hexists(redis_key, message_id):
                conn.hdel(redis_key, message_id)
                logger.debug(f"[API] Deleted message {message_id} from Redis for user {request.user.id}")
                return JsonResponse({'status': 'success'})
            else:
                logger.warning(f"[API] Message {message_id} not found in Redis for user {request.user.id}")
                return JsonResponse({'status': 'error', 'message': 'Message not found'}, status=404)
        except json.JSONDecodeError:
            logger.error("[API] Invalid JSON in DELETE request")
            return JsonResponse({'status': 'error', 'message': 'Invalid JSON'}, status=400)
        except Exception as e:
            logger.error(f"[API] Error deleting message for user {request.user.id}: {str(e)}", exc_info=True)
            return JsonResponse({'status': 'error', 'message': 'Internal server error'}, status=500)
    else:
        logger.error(f"[API] Method {request.method} not allowed for user {request.user.id}")
